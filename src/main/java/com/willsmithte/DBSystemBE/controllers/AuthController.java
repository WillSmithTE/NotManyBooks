...
public class AuthController {
    ...
    public Response register(@RequestBody RegistrationRequest registrationRequest) {
        ...
        User preExistingUser = userRepository.findByEmailRecord(registrationRequest.getEmail());
        if (preExistingUser == null) {
            User newUser = userRepository.insert(new User(registrationRequest));
    ...
    public Response login(@RequestBody LoginRequest loginRequest) {
        User preExistingUser = userRepository.findByEmailRecord(loginRequest.getEmail());
        if (preExistingUser.getPassword().equals(loginRequest.getPassword())) {
            if (preExistingUser.getEmailRecord().isConfirmed()) {
